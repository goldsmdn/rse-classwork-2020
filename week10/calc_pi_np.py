...
def calculate_pi_timeit(points):
    ...
    def calculate_pi(radius=1):
        """
        Calculates an approximated value of pi by the Monte Carlo method.
        """
        rng = np.random.default_rng(1)
        random_array = rng.random((2,points))
        r = np.sqrt(random_array[0]*random_array[0] + random_array[1]*random_array[1])
        #r = np.sqrt(random_array[0]**2 + random_array[1]**2)
        a = r[np.where(r <= radius)]
        return 4 * len( a) / points
    return calculate_pi
